...
```bash
juju deploy airbyte-k8s --channel 2/stable --trust
```
...
The database relation is satisfied, and Airbyte moves on to check its connection to Temporal:
...
Airbyte offloads workflow execution to Temporal, so Temporal must be operational before Airbyte can run syncs. First, bring Temporal up by relating it to its own backends:
...
Once `temporal-k8s` and `temporal-admin-k8s` reach `active`, create the `default` Temporal namespace that Airbyte uses:

```bash
juju run temporal-admin-k8s/0 cli args="operator namespace --namespace default create"
```

Then relate Airbyte to Temporal, so that it receives the Temporal server address:

```bash
juju integrate airbyte-k8s:temporal-host-info temporal-k8s:temporal-host-info
```

The Temporal relation is satisfied, and Airbyte moves on to check its object storage:
...
With PostgreSQL, Temporal and MinIO related, the `airbyte-bootloader` container runs its one-time initialization; you may briefly see `waiting for airbyte-auth-secrets` while it creates the auth secret, after which `airbyte-k8s/0` settles to `active`.
...
airbyte-k8s         v2.0.0                 active      1  airbyte-k8s         2/stable          37  10.x.x.x  no
...
temporal-admin-k8s  1.23.1                 active      1  temporal-admin-k8s  1.23/stable       28  10.x.x.x  no
temporal-k8s        1.23.1                 active      1  temporal-k8s        1.23/stable       68  10.x.x.x  no
...
postgresql-k8s:database            airbyte-k8s:db                        postgresql_client   regular
