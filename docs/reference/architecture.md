...
The Canonical Data Mesh operates as an integrated lifecycle where data flows through distinct stages, each handled by a dedicated component:
...
- **Authentication**: the Canonical Identity Platform provides single sign-on across the user-facing components.
...
- PostgreSQL to Trino: source databases become Trino catalogs dynamically.
...
- Trino to DataHub: catalog metadata is ingested on a schedule.
